...
# vector of length n.
#
# Inputs sa and b0 specify the prior variance and mean of the
# coefficients. Input logodds is the prior log-odds of inclusion for
# each variable. It must be a vector of length p. Note that a residual
# variance parameter (sigma) is not needed to model a binary
# outcome. See function 'updatestats_varbvsbin' for more information
# about input 'stats'.
#
...
varbvsbinupdate <- function (X, sa, b0, logodds, stats, alpha0, mu0, Xr0,
                             updates) {
  ...
  out <- .Call(C_varbvsbinupdate_Call,X = X,sa = as.double(sa),
               ...
               xy = as.double(stats$xy),xd = as.double(stats$xd),
               alpha = alpha,mu = mu,Xr = Xr,i = as.integer(updates - 1))
