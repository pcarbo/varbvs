...
vignette: >
  %\VignetteIndexEntry{Leukemia demo}
...
Carbonetto, P., *et al* (2016) Fast variable selection for genome-wide
association studies and other large-scale regression applications.
*Forthcoming*.
...
knitr::opts_chunk$set(collapse = TRUE,comment = "#",fig.width = 6.75,
                      fig.height = 5.25,fig.align = "center",
                      fig.cap = "&nbsp;")
```

## Vignette parameters

Begin by loading a few packages into the R environment.
...
Next, specify settings that control the data analysis.
...
Next, run 20-fold cross-validation to select the largest setting of the 
...
settings of *lambda*. (Do not show the intercept.) Only label the curves for 
the variables that are selected at the optimal settingof *lambda* 
("lambda.opt"").
