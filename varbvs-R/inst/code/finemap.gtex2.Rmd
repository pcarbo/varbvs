...
In this example, we consider fine-mapping for a slightly more
complicated scenario in which multiple SNPs are independently
associated with a (gene expression) quantitative trait within a single
locus. Despite the added complexity, the simple fine-mapping scheme
seems to work well for this example, although some potential issues
could occur in similar examples.
...
on the coefficients. We find that multiple SNPs in this locus are
independently associated with gene expression, even when the prior is
very conservative (that is, 1 SNP is associated with the gene
expression trait *a priori*).
...
create.finemap.plot <- function (pip, r, markers, plot.title = NULL) {
  ...
  return(ggplot(mapping = aes(x = pos,y = pip,color = r),
    ...
    geom_point(data = dat,size = 2,shape = 20) +
...
do.call("plot_grid",c(plots,list(nrow = 4,ncol = 1)))
```

In these plots, the number in the parentheses gives the number of SNPs
in the credible SNP set.

None of the credible sets overlap, so in this particular example it is
safe to treat the independent associations as implicating distinct
causal variants.

```{r}
sum(sapply(cr,length))
length(unique(do.call("c",cr)))
```
