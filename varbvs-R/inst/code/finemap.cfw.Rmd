...
The aim of this analysis is to try out a simple fine-mapping idea when
the effect sizes are reasonably large, but there is widespread linkage
disequilibrium famong SNPs in the locus. We can also build on this
analysis to test out other ideas for fine-mapping (that is, estimating
credible sets for the location of the causal variant near an
identified assocation).
...
out1  <- varbvsproxybf(X,NULL,y,fit,i,vars1) 
```
...
out2  <- varbvsproxybf(X,NULL,y,fit,j,vars2)
```
...
```{r}
cred1 <- vars1[get.credible.set(out1$BF[,1],credint.prob)]
cred2 <- vars2[get.credible.set(out2$BF[,1],credint.prob)]
