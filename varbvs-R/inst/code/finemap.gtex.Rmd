...
BF   <- varbvsproxybf(X,Z,y,fit,i,vars)
# out <- varbvsindep(fit,X,Z,y)
```
...
Get the credible SNP sets for the top SNP (using the first
hyperparameter setting only).

```{r}
cred.snps <- vars[get.credible.set(BF[,1],credint.prob)]
print(length(cred.snps))
```
